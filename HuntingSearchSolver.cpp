...
HSResult HuntingSearchSolver::Solve(
	const HuntingSearchSpace& SearchSpace, HSLogging Logging, const char* OutputFile)
{
	HSResult Result;
	// Initialize buffer
	if (Logging & HSLogging::HSLogToResult) 
	{
		Result.m_Log.reserve(200);
	}
	std::ofstream Output;
	if (Logging & HSLogging::HSLogToFile)
	{
	...
	while (Iter-- > 0)
	{
		...
		if (Logging & HSLogging::HSLogToResult)
			LogHuntersPositions(HG, SearchSpace, Result);
		...
		for (int j = 0; j < Pos.size(); ++j) 
		{
			...
			Pos[j] = RandomFloat() * std::abs(
				SearchSpace.m_DesignSpace[j].second - SearchSpace.m_DesignSpace[j].first) 
					+ SearchSpace.m_DesignSpace[j].first;
		}
		HG[i] = Hunter(Pos);
	}
...
{
	const int LeaderIndex = GetLeadersIndex(HG, SearchSpace);
	Output.m_Log.push_back(std::vector<DecisionVars>(HG.size() + 1));
	Output.m_Log.back()[0] = HG[LeaderIndex].GetHunterPosition();
	for (int i = 0; i < HG.size(); ++i)
	{
		//if (i == LeaderIndex)
		//	continue;
		Output.m_Log.back()[i + 1] = HG[i].GetHunterPosition();
	}
}
